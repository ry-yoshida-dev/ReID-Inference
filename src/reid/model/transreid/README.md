...
This subtree integrates **TransReID** (Transformer-based ReID) by prepending the vendored [`transreid_pytorch`](../../external/TransReID-SSL/transreid_pytorch) tree to `sys.path` and building a backbone registered in [`TransReID._import_backbone`](model.py). Weights are loaded with backbone-specific `load_param` (including positional embedding resize controlled by `hw_ratio`).
...
```bash
PYTHONPATH=src python3 -m reid.model.transreid.model \
...
| Flag | Default | Description |
| ---- | ------- | ----------- |
| `--model-name` | `vit_small_patch16_224_TransReID` | Backbone key in `_import_backbone` (ViT/DeiT/Swin variants). |
