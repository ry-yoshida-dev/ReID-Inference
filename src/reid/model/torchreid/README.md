...
This subtree integrates **[Torchreid](https://kaiyangzhou.github.io/deep-person-reid/)** from the vendored [deep-person-reid](https://github.com/KaiyangZhou/deep-person-reid) tree: `build_model` + `load_pretrained_weights`. `num_classes` must be consistent with the checkpoint when the classifier head is present.
...
```bash
PYTHONPATH=src python3 -m reid.model.torchreid.model \
...
params = backend.parameter_class(
    weights_path=str(Path("weights/osnet_ain_ms_d_c.pth")),
    batch_size=16,
