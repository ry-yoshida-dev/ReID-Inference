...
from ...external import FAST_REID_IMPORT_ROOT, FAST_REID_ROOT, ensure_syspath
...
class FastReID(ReIDEncoder[ReIDParameters]):
    """
    Vendored FastReID: MOT17 YACS + build_model + Checkpointer.

    Attributes:
    ----------
    Same as ReIDEncoder; model_name is YAML stem under vendored configs/MOT17.
    """
    ...
    def _load_model(self) -> torch.nn.Module:
        ...
        -------
        torch.nn.Module
            build_model(cfg). CPU if no CUDA (setup_cfg).
        """
        ...
        # MOT17 YAML stems match the vendored fast_reid configs used for MOT-style weights.
        config_file = str(FAST_REID_ROOT / "configs" / "MOT17" / f"{model_name}.yml")
        if not Path(config_file).is_file():
            raise FileNotFoundError(
                f"config_file not found in {config_file}. {model_name} might not be a supported model for FastReID."
            )
