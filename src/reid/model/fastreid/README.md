...
This subtree integrates **[FastReID](https://github.com/JDAI-CV/fast-reid)** via the pip package [`fast-reid`](https://github.com/ry-yoshida-dev/fast-reid) (`fast_reid.fastreid`): bundled YACS config under `configs/MOT17/<model_name>.yml`, `build_model`, and `Checkpointer`. `model_name` is the **YAML stem** (e.g. `sbs_S50`).
...
| Flag | Default | Description |
| ---- | ------- | ----------- |
| `--model-name` | `sbs_S50` | Config stem under packaged `fast_reid/configs/MOT17/`. |
