...
This package implements **ReID encoders** on top of [TorchModules](https://github.com/ry-yoshida-private/TorchModules): preprocessing, optional half precision, and batched inference. Three backends are wired in—**Torchreid** ([deep-person-reid](https://github.com/KaiyangZhou/deep-person-reid)), **FastReID** ([fast-reid](https://github.com/JDAI-CV/fast-reid)), and **TransReID** (vendored [TransReID-SSL](https://github.com/damo-cv/TransReID-SSL) PyTorch code). Vendored trees live under [`external/`](external/README.md); paths are adjusted at runtime via `sys.path`.
...
| [`external/`](external/README.md) | Vendored upstream repos; not modified for day-to-day inference API. |
